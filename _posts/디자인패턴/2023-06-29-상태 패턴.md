...
* 첫 번째로, Boost에서는 두 종류의 상태 머신 라이브러리를 지원한다.

  * 하나는 Boost.MSM
  
  * 다른 하나는 Boost.Statechart

  * Boost가 아니여도 많은 상태 머신 라이브러리들이 있을 것이다...


* 두 번째로, 단순한 상태 머신 이상의 고차원 상태 머신들이 많이 있다는 것을 염두한다.

  * 예를 들어 많은 라이브러리들이 `계층적 상태 머신`을 지원한다.
  * 계층적 상태 머신이란 수많은 하위 계층의 상태들을 가질 수 있다.

* 마지막으로 현대의 상태 머신은 과거의 전통적인 상태 머신 디자인 패턴과 비교해 훨씬 복잡하고 정교하다.

* 전통적인 방식은 중복된 API, 자기 자신을 삭제하는 문제등 바람직하지 않은 특성들을 가진다.